"""Shell agent tools — `run_shell`, `run_shell_background`,
`read_process_output`.

Background processes live in a module-level `PROCESS_REGISTRY` for the
duration of the session; their stdout/stderr go to tempfiles so polling
`read_process_output` always returns the full accumulated output.

`list_processes` from the legacy surface is dropped in M1 — the single
session doesn't lose process IDs, and the agent rarely needs the listing.
It can return via a Phase-2 evidence trigger.
"""
...
"""Session-scoped registry of background processes."""
...
def cleanup_processes() -> None:
    """Best-effort teardown of background processes + their tempfiles.

    Called at session-end by the Runtime slice (Slice 4); safe to call now
    from tests. Idempotent — clears the registry on each invocation.
    """
