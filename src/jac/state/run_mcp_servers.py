...
class RunMcpServersRepo:
    ...
    async def list_active_for_run(
        ...
        """Return enabled run_mcp_servers rows joined with enabled mcp_servers."""
    ...
    async def list_active_for_run_with_details(
        ...
        """Return enabled run_mcp_servers joined with mcp_servers details."""
        params: list[object] = [run_id]
        role_filter = ""
        if agent_role is not None:
            role_filter = "AND (rm.agent_role = ? OR rm.agent_role IS NULL)"
    ...
    async def toggle(
        ...
        await self._connection.commit()
        cursor = await self._connection.execute(
            "SELECT * FROM run_mcp_servers WHERE id = ?",
            (row_id,),
        )
