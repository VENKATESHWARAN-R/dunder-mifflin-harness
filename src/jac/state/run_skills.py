...
class RunSkillsRepo:
    """Per-run enablement layer over `skills`."""
    ...
    async def create(
        ...
        *,
        ...
        await self._connection.execute(
            """
            INSERT INTO run_skills (id, run_id, skill_id, agent_role, enabled, toggled_at)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
    ...
    async def toggle(self, row_id: str, *, enabled: int) -> RunSkillRow | None:
        now = _now()
        await self._connection.execute(
            "UPDATE run_skills SET enabled = ?, toggled_at = ? WHERE id = ?",
            (enabled, now, row_id),
        )
        await self._connection.commit()
        cursor = await self._connection.execute(
            "SELECT * FROM run_skills WHERE id = ?", (row_id,)
        )
